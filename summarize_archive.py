...
# --- 24/7 LOGGING SETUP ---
# This ensures logs are saved to a file that never grows beyond 5MB.
log_formatter = logging.Formatter("%(asctime)s [%(levelname)s] %(message)s")
log_file = "system.log"

# Create a handler that rotates the log file at 5MB, keeping 2 backups
file_handler = RotatingFileHandler(log_file, maxBytes=5_000_000, backupCount=2)
file_handler.setFormatter(log_formatter)

# Create a handler to still show logs in the terminal window
